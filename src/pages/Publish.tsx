...
export default function Publish() {
  ...
  return (
    ...
    <div className="flex justify-center w-full">
        ...
        <input type="text" onChange={(e)=>{settitle(e.target.value)}} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:outline-blue-500 focus:border-blue-500 block w-full p-2.5 mb-5 " placeholder="Title" />
        <Textarea setcontent={setcontent}/>
        
        <button onClick={title.length>1&&content.length>1?async ()=>{const response= await axios.post(`${BACKEND_URL}/api/v1/blog`,{title,content},{headers:{Authorization:localStorage.getItem("token")}}); navigate(`/blog/${response.data.id}`)
    }:()=>alert("type something")}  type="submit" className="inline-flex items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg focus:ring-4 focus:ring-blue-200 dark:focus:ring-blue-900 hover:bg-blue-800">
       Publish Blog
   </button>
      </div>
...
function Textarea({setcontent}:{setcontent:Function}){
    return <>
<form>
   <div className="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 ">
       <div className="flex items-center justify-between px-3 py-2 border-b ">
           <div className="flex flex-wrap items-center divide-gray-200 sm:divide-x sm:rtl:divide-x-reverse ">
               <div className="flex items-center space-x-1 rtl:space-x-reverse sm:pe-4">
                   <button type="button" className="p-2 text-gray-900 rounded cursor-pointer hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-200">
       ...
       <div className="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-200">
           ...
           <textarea  onChange={(e)=>{setcontent(e.target.value)}}id="editor" rows={8} className="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-gray-200 focus:ring-0 focus:outline-blue-500" placeholder="Write content here " required ></textarea>
       </div>
   </div>
